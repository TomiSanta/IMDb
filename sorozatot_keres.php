...
<form action="sorozatot_kereshozzaadasa.php" method="POST">

    <label>Felhasználó neve:</label><label for="sorozatot_keres"></label>
    <input type="text" id="felhasznalonev" name="felhasznalonev" placeholder="Felhasználó neve" required/>

    <label>Cím:</label><label for="sorozatot_keres"></label>
...
if ( mysqli_select_db($conn, 'imdb') ) {    // ha sikeres az adatbázis kiválasztása
    ...
    $sql = "SELECT felhasznalonev, cim FROM sorozatot_keres"; // ez csak egy string, még nem hajtódik végre
    ...
    while ( ($current_row = mysqli_fetch_assoc($res))) {    // most asszociatív tömbként kezeljük a sorokat
        echo '<form action="sorozatot_keresmodositas.php" method="post">';
        echo '<tr>';
